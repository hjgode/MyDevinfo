...
drawPixmap(PluginInstance *This)
{
    ...
    {
        int pixmap_width, pixmap_height, dest_x, dest_y;
        gdk_window_get_size((GdkWindow *)nullPluginGdkPixmap, &pixmap_width, &pixmap_height);
        dest_x = This->width/2 - pixmap_width/2;
        ...
        if (dest_x >= 0 && dest_y >= 0)
        {
            ...
            fprintf(stderr, "%s %d,%d %dx%d\n",
                    __func__, dest_x, dest_y, pixmap_width, pixmap_height);
            XCopyArea(This->display, GDK_WINDOW_XWINDOW(nullPluginGdkPixmap) , This->window, gc,
                0, 0, pixmap_width, pixmap_height, dest_x, dest_y);
